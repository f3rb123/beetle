...
def _findings(*binaries, main="Runner"):
    rows = bp.build_table(list(binaries), main_binary=main)
...
def test_no_missing_protection_finding_is_ever_high():
    # A vendor framework's missing canary is a real hardening gap, not a directly exploitable
    # app weakness. HIGH would overstate it — and RUN 15 needs a defensible score.
    _rows, (findings, _sup) = _findings(MAIN, REAL_FRAMEWORK)
